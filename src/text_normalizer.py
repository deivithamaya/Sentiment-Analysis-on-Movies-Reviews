...
def stem_text(text: str) -> str:
    ...
    Return:
        str
            Output string.
    """
    return " ".join(nltk.stem.PorterStemmer().stem(token) for token in nltk.tokenize.word_tokenize((text)))
    #return " ".join(nltk.stem.PorterStemmer().stem(token) for token in ToktokTokenizer().tokenize((text.replace("\n",""))))
...
def lemmatize_text(text: str) -> str:
    ...
    """
    sentences = nlp(text)
    return " ".join(word.lemma_ for word in sentences)
...
def remove_special_chars(text: str, remove_digits: Optional[bool] = False) -> str:
    ...
    chards = []
    #chards = [token for token in tokens if token.isalpha()]
    for token in tokens:
        for character in token:
            if not character.isalpha():  chards.append(character)
    chards.append("\"")
    if remove_digits:
        chards += [str(number) for number in range(10)]
    for chard in chards:
        if chard != "." : text = text.replace(chard, "").replace("  ", " ")
        else : text = text.replace(".", " ")
    if not remove_digits : text = text.strip() 
    return text
...
def remove_extra_whitespace(text: str) -> str:
    ...
    return " " .join(ToktokTokenizer().tokenize(text))
    #return text.strip()
